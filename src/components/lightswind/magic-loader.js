...
const MagicLoader = ({ size = 200, particleCount = 1, speed = 1, hueRange = [0, 360], className, loadingProgress = 0 }) => {
    ...
    const createParticle = useCallback((centerX, centerY, tick, minSize) => {
        // Calculate position based on continuous rotation
        const rotationAngle = (globalRotationRef.current + (tick * 0.02));
        
        return {
            radius: 7,
            x: centerX + Math.cos(rotationAngle) * minSize / 2,
            y: centerY + Math.sin(rotationAngle) * minSize / 2,
            angle: rotationAngle,
            speed: 0,
            accel: 0.01,
            decay: 0.01,
    ...
    const animate = useCallback(() => {
        ...
        // Continuous rotation - not tied to loading progress
        // This ensures smooth rotation throughout the loading process
        globalRotationRef.current += (Math.PI / 120) * speed; // Slower, smoother rotation
        globalAngleRef.current += (Math.PI / 120) * speed;
    ...
    const setupCanvas = useCallback(() => {
        ...
        tickRef.current = 0;
        // Don't reset global angles - let them continue smoothly
    }, [size]);
